    ...
    <div class="container mx-auto px-4 py-4">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-4 gap-4">
                ...
                <div id="dropdownRadio"
                    ...
                    <ul class="p-3 space-y-1 text-sm text-gray-800" aria-labelledby="dropdownRadioButton">
                        @foreach (\App\Models\Jenis::orderBy('nama_jenis', 'asc')->get() as $jenis)
                            <li>
                                <a href="{{ route('pesanan.buat', ['filter_jenis' => $jenis->id_jenis, 'search' => request('search')]) }}"
                        ...
                        @endforeach
                        <li>
                            <a href="{{ route('pesanan.buat', ['search' => request('search')]) }}"
                                class="block px-4 py-2 hover:bg-teal-100 rounded transition">
                                Semua Kategori
                            </a>
                        </li>
            ...
            <div class="flex-grow w-full md:max-w-xl">
                <form action="{{ route('pesanan.buat') }}" method="GET" class="flex">
                    <input type="text" name="search" placeholder="Cari produk atau kategori..."
                        value="{{ request('search') }}"
                        class="flex-grow px-4 py-2 border rounded-l-lg focus:outline-none focus:ring-2 focus:ring-teal-500">
                    <button type="submit"
                        class="bg-teal-500 text-white px-4 py-2 rounded-r-lg hover:bg-teal-600">
                        Cari
                    </button>
                </form>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white border rounded shadow p-4 flex flex-col">
                    ...
                    <form action="{{ route('user.pesanan.tambahKeranjang', $produk->id_produk) }}" method="POST"
                        class="mt-auto">
                        @csrf
