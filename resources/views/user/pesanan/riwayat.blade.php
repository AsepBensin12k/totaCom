                        ...
                        <div
                            ...
                            <div class="text-right">
                                <p class="text-sm text-gray-500">Tanggal Pesanan: <span
                                        class="font-medium text-gray-700">{{ \Carbon\Carbon::parse($pesanan->created_at)->format('d F Y, H:i') }}</span>
                                </p>
                                    ...
                                    @forelse($pesanan->detailPesanans as $detail)
                                        <tr class="hover:bg-gray-50">
                                            <td class="py-3 px-4 whitespace-nowrap text-gray-800">
                                                {{ $detail->produk->nama_produk ?? 'Produk tidak ditemukan' }}</td>
                                            <td class="py-3 px-4 whitespace-nowrap text-gray-800">{{ $detail->qty ?? '0' }}
                                            </td>
                                            <td class="py-3 px-4 whitespace-nowrap text-gray-800">Rp
                                                {{ number_format($detail->harga ?? 0, 0, ',', '.') }}</td>
                                            <td class="py-3 px-4 whitespace-nowrap text-right text-gray-800">Rp
                                                {{ number_format(($detail->qty ?? 0) * ($detail->harga ?? 0), 0, ',', '.') }}
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" class="py-4 px-4 text-center text-gray-500">Tidak ada detail
                                                produk untuk pesanan ini.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                                <tfoot class="bg-gray-50">
                                    <tr>
                                        <td colspan="3" class="py-3 px-4 text-right font-bold text-gray-800 uppercase">
                                            Total Pesanan:</td>
                                        <td class="py-3 px-4 text-right font-bold text-green-600 text-lg">Rp
