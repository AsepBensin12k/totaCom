...
@section('content')
    ...
    <div class="space-y-8 animate-fade-in-up mb-12 mt-10">
        <div
            class="bg-gradient-to-r from-teal-500 to-teal-600 text-white p-8 rounded-lg shadow-lg flex items-center justify-between">
            <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 justify-items-center">
            ...
            <a href="{{ route('pesanan.buat') }}"
                class="block bg-white p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-teal-200 hover:border-teal-500 w-full max-w-sm">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            ...
            <div class="space-y-4">
                @forelse($latestOrders as $pesanan)
                    <a href="{{ route('pesanan.riwayat') }}"
                        class="block p-4 bg-gray-50 rounded-md border border-gray-100 hover:bg-teal-50 transition-colors duration-200">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="text-right">
                                <p class="text-sm text-gray-500">
                                    {{ \Carbon\Carbon::parse($pesanan->tanggal)->diffForHumans() }}</p>
                                <p class="font-bold text-gray-700">Rp
                                    {{ number_format($pesanan->detailPesanans->sum(function ($d) {return ($d->qty ?? 0) * ($d->harga ?? 0);}),0,',','.') }}
                                </p>
                            </div>
                        </div>
                    </a>
                ...
                <div class="text-center mt-4">
                    <a href="{{ route('pesanan.riwayat') }}"
                        class="text-teal-600 hover:text-teal-800 font-semibold transition-colors duration-200">Lihat Semua
                        Pesanan <i class="fas fa-arrow-right text-sm ml-1"></i></a>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                    ...
                    <div
                        ...
                        @if ($produk->gambar)
                            <img src="{{ asset('storage/' . $produk->gambar) }}"
                                class="w-full h-40 object-cover rounded-t-lg" alt="{{ $produk->nama_produk }}">
                        @else
                            <div
                                class="w-full h-40 bg-gray-200 rounded-t-lg flex items-center justify-center text-gray-500">
                                No Image
                            </div>
                        @endif
                        <div class="p-4">
            ...
            <div class="text-center mt-6">
                <a href="{{ route('user.produk.index') }}"
                    class="bg-teal-500 text-white px-6 py-3 rounded-full text-lg font-semibold hover:bg-teal-600 transition-colors duration-200 shadow-md">
